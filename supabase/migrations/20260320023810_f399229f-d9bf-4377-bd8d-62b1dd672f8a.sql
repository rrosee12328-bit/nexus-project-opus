...
-- Remove run_id from transactional email triggers (only auth emails use run_id)
...
BEGIN
  ...
  PERFORM public.enqueue_email('transactional_emails', jsonb_build_object('to', _client_email, 'from', 'Vektiss <noreply@notify.vektiss.com>', 'sender_domain', 'notify.vektiss.com', 'subject', _subject, 'html', _html, 'text', 'You have a new message from ' || COALESCE(_sender_name, 'Vektiss') || ': ' || _preview, 'purpose', 'transactional', 'label', 'new_message_notification', 'message_id', _msg_id, 'queued_at', now()::text));
  RETURN NEW;
END;
$function$;
...
BEGIN
  ...
  PERFORM public.enqueue_email('transactional_emails', jsonb_build_object('to', _client_email, 'from', 'Vektiss <noreply@notify.vektiss.com>', 'sender_domain', 'notify.vektiss.com', 'subject', _subject, 'html', _html, 'text', 'Hi ' || COALESCE(_client_name, 'there') || ', a payment of $' || NEW.amount::text || ' for ' || _month_name || ' ' || NEW.payment_year::text || ' has been confirmed.', 'purpose', 'transactional', 'label', 'payment_confirmation', 'message_id', _msg_id, 'queued_at', now()::text));
  RETURN NEW;
END;
$function$;
...
BEGIN
  ...
  PERFORM public.enqueue_email('transactional_emails', jsonb_build_object('to', _client_email, 'from', 'Vektiss <noreply@notify.vektiss.com>', 'sender_domain', 'notify.vektiss.com', 'subject', _subject, 'html', _html, 'text', 'Hi ' || COALESCE(_client_name, 'there') || ', your project "' || NEW.name || '" has been updated.', 'purpose', 'transactional', 'label', 'project_update', 'message_id', _msg_id, 'queued_at', now()::text));
  RETURN NEW;
END;
$function$;
