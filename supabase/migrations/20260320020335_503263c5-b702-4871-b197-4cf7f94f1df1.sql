...
BEGIN
  ...
  _html := '<!DOCTYPE html><html><head><meta charset="utf-8"></head>'
    ...
    || '<div style="background-color: #f5f5f5; border-left: 4px solid hsl(225, 100%, 61%); padding: 16px; border-radius: 6px; margin: 0 0 25px;">'
    ...
    || 'style="display: inline-block; background-color: hsl(225, 100%, 61%); color: #ffffff; font-size: 14px; font-weight: 600; border-radius: 6px; padding: 12px 24px; text-decoration: none;">View Messages</a>'
...
CREATE TRIGGER trg_email_on_new_message
  AFTER INSERT ON public.messages
  FOR EACH ROW
  EXECUTE FUNCTION public.email_on_new_message();
