...
serve(async (req) => {
  ...
  try {
    ...
    if (header.status === "paid") {
      throw new Error("Paid invoices can't be voided. Use Duplicate to issue a new draft and refund in Stripe Dashboard.");
    }
    ...
    if (header.stripe_invoice_id) {
      try {
        if (header.status === "draft") {
          await stripe.invoices.del(header.stripe_invoice_id);
        } else {
          // open / uncollectible
          await stripe.invoices.voidInvoice(header.stripe_invoice_id);
        }
      } catch (e: any) {
        console.warn("Stripe void failed (continuing to mark void in DB):", e?.message);
      }
    }
    ...
    console.error("void-hourly-invoice error:", err);
